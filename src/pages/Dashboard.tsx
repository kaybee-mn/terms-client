...
export default function Dashboard() {
  ...
  function renderContent1() {
    ...
    return typedData[provValues.provider1].dateOptions[dateValues.date1][
      toggle
    ];
  }
  function renderContent2() {
    const toggle = showDropdowns.toggle2 ? "simplified" : "detailed";
    return typedData[provValues.provider2].dateOptions[dateValues.date2][
      toggle
    ];
  }
  function updateProv(val: string, name: string) {
    ...
    setDateSets({
      ...dateSets,
      [name]: Object.keys(typedData[val].dateOptions),
    });
    setDateValues({
      ...dateValues,
      ["date" + name.charAt(name.length - 1)]: Object.keys(
        typedData[val].dateOptions
      )[0],
    });
